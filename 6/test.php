...
$m = 0;
        ...
        $totalq = ($w+$r+$m)*0.8;
        if ($totalq > $r) {
            $finalize = "Тест завален...";
        }
        elseif ($totalq == $r) {
            $finalize = "Вы прошлив тютелька в тютелтку. Подготавливайтесь получше. Вдруг не повезет в следующий раз?";
        }
        elseif ($r === ($w+$r+$m)) {
            $finalize = "Чистая, совершенная пятерочка! Так держать!";
        }
        elseif ($totalq < $r) {
            $finalize = "Тест пройден!";
        }

        echo "</table>";
        echo "<h4 class=\"correct\">Правильных ответов: ".$r."</h4>";
        echo "<h4 class=\"missed\">Пропущено: ".$m."</h4>";
        echo "<h4 class=\"wrong\">Ошибов: ".$w."</h4>";
        echo "<hr3>".$finalize."</hr3>";
    }
}
else {
        $i = 0;
